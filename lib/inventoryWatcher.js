...
const MIN_SEND_GAP_MS = 12_000; // 5 messages per minute max
const MAX_LINES_PER_MESSAGE = 15;
...
// Per-event flood protection: when one event produces N+ alerts of the same type
// in a single message, collapse them to one summary field that highlights the
// most actionable example (biggest drop / cheapest price).
const FLOOD_THRESHOLD = 5;

function groupByEvent(items) {
  const m = new Map();
  for (const it of items) {
    const k = it._evt.event_name || '(unknown event)';
    if (!m.has(k)) m.set(k, []);
    m.get(k).push(it);
  }
  return m;
...
function dropSummaryField(group) {
  const worst = [...group].sort(
    (a, b) => (a.newPrice - a.oldPrice) / a.oldPrice - (b.newPrice - b.oldPrice) / b.oldPrice,
  )[0];
  const tag = worst.tag === 'standard' ? 'STD' : 'RSL';
  const pct = pctChange(worst.oldPrice, worst.newPrice);
  return {
    name: `🔴  ${group.length}× drops  ·  ${tag}  ·  ${eventName(worst)}`,
    value: `Biggest: SEC **${worst.section}** Row **${worst.row}** · ${seatsTag(worst)}\n` +
           `~~${fmt$(rawPrice(worst, worst.oldPrice))}~~ → **${fmt$(rawPrice(worst, worst.newPrice))}** (${pct})${eventLinkSuffix(worst)}`,
    inline: false,
  };
}

function newStdSummaryField(group) {
  const cheapest = [...group].sort((a, b) => a.price - b.price)[0];
  return {
    name: `🟢  ${group.length}× NEW STANDARD  ·  ${eventName(cheapest)}`,
    value: `Cheapest: SEC **${cheapest.section}** Row **${cheapest.row}** · ${seatsTag(cheapest)}\n` +
           `**${fmt$(rawPrice(cheapest, cheapest.price))}**${eventLinkSuffix(cheapest)}`,
    inline: false,
  };
}

function undercutSummaryField(group) {
  const cheapest = [...group].sort((a, b) => a.price - b.price)[0];
  const pct = pctChange(cheapest.sectionLow, cheapest.price);
  return {
    name: `🟡  ${group.length}× undercuts  ·  RSL  ·  ${eventName(cheapest)}`,
    value: `Cheapest: SEC **${cheapest.section}** Row **${cheapest.row}** · ${seatsTag(cheapest)}\n` +
           `**${fmt$(rawPrice(cheapest, cheapest.price))}** vs sec low ${fmt$(rawPrice(cheapest, cheapest.sectionLow))} (${pct})${eventLinkSuffix(cheapest)}`,
    inline: false,
  };
}

function buildBucketFields(items, individualFn, summaryFn) {
  if (!items.length) return [];
  const fields = [];
  for (const group of groupByEvent(items).values()) {
    if (group.length >= FLOOD_THRESHOLD) {
      fields.push(summaryFn(group));
    } else {
      for (const item of group) fields.push(individualFn(item));
    }
  }
  return fields;
}

function takeFromFront(arr, n) {
...
async function sendCombined() {
  ...
  const dropShown = takeFromFront(p.drops, dropTake);
  const undShown  = takeFromFront(p.undercuts, undTake);
  const stdShown  = takeFromFront(p.newStd, stdTake);

  // Each alert becomes its own embed field — but if 5+ items of one type belong
  // to the same event in this batch, collapse to ONE summary field (flood protection).
  const fields = [
    ...buildBucketFields(dropShown, dropField, dropSummaryField),
    ...buildBucketFields(undShown,  undercutField, undercutSummaryField),
    ...buildBucketFields(stdShown,  newStdField, newStdSummaryField),
  ];
...
function pushPending(state, type, item) {
  ...
  if (arr.length > PENDING_CAP) {
    ...
    console.warn(`[inventoryWatcher] pending ${type} cap reached — dropped ${dropped} oldest (lifetime dropped=${state.droppedAlerts})`);
  }
}
    ...
    if (!prev) {
      if (!isBaseline) {
        if (data.tag === 'standard') {
          ...
          if (!isDuplicate(state, item, 'newStd')) {
          ...
          if (secLow !== Infinity && data.price < secLow && data.price <= secLow * (1 - UNDERCUT_PCT)) {
            ...
            if (!isDuplicate(state, item, 'undercut')) {
      ...
      if (data.price < baseline) {
        ...
        if (drop >= MIN_DROP_ABS && dropPct >= MIN_DROP_PCT) {
          ...
          if (!isDuplicate(state, item, 'drop')) {
